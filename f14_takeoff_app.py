...
def load_perf() -> pd.DataFrame:
    ...
    return perf
            ...
            for c in ["arm_in", "unit_weight_lb", "max_qty"]:
                df[c] = pd.to_numeric(df[c], errors="coerce")

            st.caption(f"Loaded stations file: {path}")
            return df
    ...
    if last_err:
        st.warning(
            "Add f14_stations.csv to enable stores list and CG. Expected columns: "
            "station_id,name,arm_in,unit_weight_lb,max_qty "
            f"(last error: {last_err})"
        )
    else:
        st.warning(
            "Add f14_stations.csv to enable stores list and CG. Expected columns: station_id,name,arm_in,unit_weight_lb,max_qty"
        )
    return None
...
def compute_performance(perfdb: pd.DataFrame, heading_deg: float, tora_ft: float, toda_ft: float, asda_ft: float,
    ...
    limiting = "ASD" if asd_eff >= agd_eff else "AGD"
        ...
        return PerfResult(v1, vr, v2, vs, flap, thrust_mode, req_ft, avail_ft, limiting, eff * 100.0)
...
("Takeoff Flaps", ["Auto-Select", "UP", "MANEUVER", "FULL"], index=0)
