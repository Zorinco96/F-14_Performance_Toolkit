...
# =============================================================================
# Physics-backed Performance Cards (drop-in section)
# Safe to append at end of file. Uses your existing session_state inputs if set.
# =============================================================================

# Bridge wrappers from core (safe getattr so this won't crash if missing)
...
# -------- small getters with tolerant defaults (use your UI state if present)
...
# Defaults aligned to your constants up top
...
with st.expander("Takeoff — Vs / Vr / VLOF / V2 & distances", expanded=True):
    if not callable(perf_takeoff):
        ...
        # Inputs (pull from session if you already set them; otherwise fallbacks)
        gw_lb         = _f("gw_takeoff_lb", DEFAULT_GTOW)
        field_elev_ft = _f("dep_elev_ft", _series_max(airports, "threshold_elev_ft") or 0.0)
        oat_c         = _f("oat_c", 15.0)
        # Wind components: prefer parsed headwind in your UI; fallback 0
        headwind_kts  = _f("headwind_kts", 0.0)
        runway_slope  = _f("runway_slope", 0.0)            # +uphill, -downhill (ft/ft)
        thrust_mode   = str(_get("thrust_mode", "MAX"))     # tie to your DERATE/AB if you have one
        mode_flag     = "DCS"                               # or switch to "FAA" with a sidebar toggle later
        # Loadout (map your station UI later; for now keep empty to use 'clean' deltas)
        stores_list   = _get("stores_list_takeoff", [])     # e.g., ["2xPhoenix","PylonPair"]
        ...
        c1.metric("Vs (kt)",   f"{res['Vs_kts']:.1f}")
        c2.metric("Vr (kt)",   f"{res['VR_kts']:.1f}")
        c3.metric("VLOF (kt)", f"{res['VLOF_kts']:.1f}")
        c4.metric("V2 (kt)",   f"{res['V2_kts']:.1f}")

        c5, c6, c7 = st.columns(3)
        c5.metric("Ground roll (ft)",        f"{res['GroundRoll_ft']:.0f}")
        c6.metric("Distance to 35 ft (ft)",  f"{res['DistanceTo35ft_ft']:.0f}")
        c7.metric("Time to 35 ft (s)",       f"{res['TimeTo35ft_s']:.1f}")

        # Optional: show JSON payload for debugging
