...
## (Optional) GitHub Actions: `/.github/workflows/tests.yml`
...
2. Path: `f14_takeoff_core.py` → paste the content above → **Commit directly to main**.
3. Add directory `tests/` → new file `tests/test_core.py` → paste → **Commit**.
4. (Optional) Add workflow at `.github/workflows/tests.yml`.
5. Create an annotated tag by making a **Release** titled `v1.2.0-core-skel`.

### Next

Once merged, the UI can import the core and keep running with placeholders. We’ll proceed to M2 to wire real table interpolation and field-length logic, validated against your Henderson/DCS snapshots.
